...
router.get("/youtube/search", async (req, res) => {
  ...
  if (!parsed.success) {
    res.status(400).json({
      error: "A song search query is required.",
    });
    return;
  }
  ...
  try {
    ...
    const candidates = searchItems
      .map((item, index) => {
        ...
        const lower =
          `${title} ${channelTitle}`.toLowerCase();

        const officialBoost =
          /official|vevo|music video|audio/.test(
            lower,
          )
            ? 25
            : 0;
